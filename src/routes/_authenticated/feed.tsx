  ...
  async function handleSubmit(back: File, front: File, caption: string) {
    ...
    try {
      ...
      const [{ error: e1 }, { error: e2 }] = await Promise.all([
        supabase.storage.from("posts").upload(backPath, back, { contentType: "image/jpeg", upsert: false }),
        supabase.storage.from("posts").upload(frontPath, front, { contentType: "image/jpeg", upsert: false }),
      ]);
    ...
    <>
      <section className="max-w-2xl mx-auto px-4 md:px-6 py-10 md:py-14 space-y-8 relative">
        <header className="text-center space-y-3">
          <p className="text-[10px] uppercase tracking-[0.42em] text-muted-foreground">The Atelier Feed</p>
          <h1 className="font-serif text-4xl md:text-5xl text-ink leading-tight">Today's looks, in real time</h1>
        ...
        {isLoading && (
          <div className="space-y-6">
            {[0, 1].map((i) => (
              <div key={i} className="rounded-3xl border border-porcelain/60 bg-background/60 backdrop-blur overflow-hidden">
        ...
        {!isLoading && data?.has_posted_today && posts.length === 0 && (
          <div className="rounded-3xl border border-dashed border-porcelain/70 p-10 text-center">
            ...
            <p className="mt-2 text-sm text-stone">As your circle posts, their looks will land here.</p>
          </div>
        )}
        ...
        {locked && (
          <>
            ...
            <div className="space-y-6 pointer-events-none select-none blur-sm opacity-60">
              {[0, 1].map((i) => (
                <div key={i} className="rounded-3xl border border-porcelain/60 bg-background/70 overflow-hidden">
            ...
            <div className="fixed inset-x-0 bottom-0 top-16 z-30 flex items-center justify-center px-6">
              ...
              <div className="relative max-w-md w-full rounded-[2rem] border border-porcelain/70 bg-background/80 backdrop-blur-2xl shadow-atelier-float p-8 md:p-10 text-center space-y-6">
                ...
                <div className="space-y-3">
                  <p className="text-[10px] uppercase tracking-[0.42em] text-muted-foreground">Anti-Lurker Gate</p>
                  <h2 className="font-serif text-3xl md:text-4xl text-ink leading-tight">
                    Your friends' looks are locked.
                  </h2>
                  <p className="text-sm text-stone max-w-sm mx-auto leading-relaxed">
                    Reveal your Atelier Blueprint to unlock today's stream. One mirror selfie, one face, one moment — and the feed is yours.
                  </p>
                </div>
                <button
                  type="button"
