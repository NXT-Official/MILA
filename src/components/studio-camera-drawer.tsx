...
const COPY: Record<StudioCameraMode, { title: string; description: string }> = {
  "look-analysis": {
    ...
    description: "Step back so I can see head to toe. I'll tell you what's singing and what to swap.",
  },
  "dupe-hunter": {
    title: "Hunt the luxury dupe",
    description: "Snap an inspiration piece — designer bag, coat, shoe. I'll extract the silhouette and surface budget-friendly alternatives.",
  },
};
        ...
        {postingOpen ? (
          <>
            <SheetHeader className="text-center space-y-2 mb-6">
              ...
              <p className="text-[10px] uppercase tracking-[0.42em] text-muted-foreground">Daily Drop</p>
        ...
        >
          <span className="flex items-center gap-3">
            ...
            <span className="flex flex-col">
              <span className="text-[9px] uppercase tracking-[0.32em] text-stone">Daily Drop</span>
              <span className="font-serif text-base text-ink">Post Today's OOTD</span>
            </span>
          </span>
          <span className="text-[10px] uppercase tracking-[0.28em] text-stone group-hover:text-ink">Dual capture →</span>
        </button>
        </>
        )}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? "text-ink font-semibold" : "text-muted-foreground hover:text-foreground",
                )}
              >
                {m.label}
              </button>
              ...
              {dupeLoading && (
                <div className="flex flex-col items-center justify-center gap-4 py-12">
                  <div className="relative h-16 w-16">
                    ...
                    <Loader2 className="absolute inset-0 m-auto h-5 w-5 text-atelier-champagne animate-spin" strokeWidth={1.25} />
                  </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4 rounded-2xl border border-porcelain/60 bg-background/60 backdrop-blur p-4">
                    ...
                    <div className="min-w-0">
                      <p className="text-[9px] uppercase tracking-[0.32em] text-muted-foreground">Inspiration</p>
                      <p className="font-serif text-base text-ink truncate">{dupeResult.inspiration.name}</p>
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        {dupeResult.inspiration.silhouette_tags.slice(0, 3).map((t) => (
                          <span key={t} className="text-[9px] uppercase tracking-[0.2em] text-stone px-1.5 py-0.5 rounded-full border border-porcelain/60">
                            {t}
                          </span>
                        ))}
                      </div>
                  ...
                  {dupeResult.dupes.length > 0 ? (
                          ...
                          <CarouselItem key={d.id} className="pl-3 basis-[78%] sm:basis-[52%]">
                            <div className="h-full rounded-2xl border border-porcelain/60 bg-background/70 backdrop-blur overflow-hidden flex flex-col shadow-atelier-soft">
                              ...
                              <div className="p-4 flex-1 flex flex-col gap-3">
                                ...
                                >
                                  Shop the Dupe <ExternalLink className="h-3 w-3" strokeWidth={1.5} />
                                </a>
                              </div>
                            </div>
                          </CarouselItem>
                    ...
                    <div className="rounded-2xl border border-dashed border-porcelain/60 p-6 text-center">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">Try a cleaner background or a different angle.</p>
                    </div>
                  )}
                </div>
              )}
