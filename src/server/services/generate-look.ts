...
import { CloudflareRateLimitError, generateOutfitImage } from "@/lib/cloudflare-image.server";
...
/**
 * The one implementation of "compose today's look".
 *
 * Both clients call this: the website through `generateDailyLook` in
 * `lib/generate-outfit.functions.ts`, mobile through `POST /api/v1/look/generate`.
 * The prompt, the credit charge, and the pending-image flag live here and
 * nowhere else — a second copy is how the two clients start dressing the same
 * member differently.
 *
 * `supabase` is the **caller's own** client, so RLS applies exactly as it does
 * in the browser. `userId` is derived from a verified session by the caller,
...
/**
 * Renders the visual for an already-composed look. Separate from `generateLook`
 * because it is separately billed: the first render is covered by the credit the
 * composition charged, and `payForLookImage` is what decides that.
 */
