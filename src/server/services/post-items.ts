...
export const UpdatePostItemsInput = z.object({
 ...
 */
export async function updatePostItemsService({
  supabase,
  userId,
  input: data,
}: {
  supabase: SupabaseClient<Database>;
  userId: string;
  input: z.infer<typeof UpdatePostItemsInput>;
}): Promise<PostItem[]> {
  {
 ...
 */
/**
 * One vision call per post, not per garment. Called by the website's
 * `analyzeOutfitItems` and by `POST /api/v1/items/analyze`.
 */
export async function analyzePostItems({
  supabase,
  userId,
  input: data,
}: {
